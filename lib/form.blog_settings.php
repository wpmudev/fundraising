...
if (!class_exists('WpmuDev_HelpTooltips')) require_once WDF_PLUGIN_BASE_DIR . '/lib/external/class.wd_help_tooltips.php';
	...
	$tabs = array(
		'general' => __('General','wdf'),
		'payments' => __('Payments','wdf'),
		//'messages' => __('Messages','wdf'),
		'reset' => __('Reset','wdf')
	);
	if(!isset($_GET['tab']))
		$active_tab = 'general';
					...
					<div>
						<?php switch($active_tab) {
							case 'general' : ?>
								...
								<h3><?php echo __('General Settings','wdf'); ?></h3>
								<table class="form-table">
									<tbody>
										
										<tr valign="top">
											<th scope="row">
												<label><?php echo __('Add fundraiser directory to your page list menu?','wdf'); ?></label>
											</th>
											<td>
										...
										<?php /*?><tr valign="top">
											<th scope="row">
												<label><?php echo __('Is your fundraising panel in your sidebar?','wdf'); ?></label>
											</th>
											<td>
												<select name="wdf_settings[panel_in_sidebar]">
													<option value="no" <?php selected($settings['panel_in_sidebar'],'no') ?>>No</option>
													<option value="yes" <?php selected($settings['panel_in_sidebar'],'yes') ?>>Yes</option>
												</select>
												<?php echo $tips->add_tip(__('You can add a special fundraising panel widget that will only appear on single fundraising pages.','wdf')); ?>
											</td>
										</tr><?php */?>
										...
										<tr valign="top">
											<th scope="row">
												<label><?php _e('Fundraising Slugs','wdf'); ?></label>
											</th>
											<td>
										...
										<tr valign="top">
											<th scope="row">
												<label><?php _e('Directory Slug','wdf'); ?></label>
											</th>
											<td>
										...
										<tr valign="top">
											<th scope="row">
												<label><?php _e('Checkout Slug','wdf'); ?></label>
											</th>
											<td>
												<span class="code"><?php echo home_url().'/'.$settings['dir_slug'].'/%post_name%/'; ?></span><input id="wdf_checkout_slug" type="text" name="wdf_settings[checkout_slug]" value="<?php echo esc_attr($settings['checkout_slug']); ?>" />
											</td>
										</tr>
										
										<tr valign="top">
											<th scope="row">
												<label><?php _e('Confirmation Slug','wdf'); ?></label>
											</th>
											<td>
												<span class="code"><?php echo home_url().'/'.$settings['dir_slug'].'/%post_name%/'; ?></span><input id="wdf_confirm_slug" type="text" name="wdf_settings[confirm_slug]" value="<?php echo esc_attr($settings['confirm_slug']); ?>" />
											</td>
										</tr>
										
										<tr valign="top">
											<th scope="row">
												<label><?php _e('Activity Slug','wdf'); ?></label>
											</th>
											<td>
												<span class="code"><?php echo home_url().'/'.$settings['dir_slug'].'/%post_name%/'; ?></span><input id="wdf_activity_slug" type="text" name="wdf_settings[activity_slug]" value="<?php echo esc_attr($settings['activity_slug']); ?>" />
											</td>
										</tr>
										...
										<tr valign="top">
											...
											<td>
												<select name="wdf_settings[default_style]">
													<option <?php selected($settings['default_style'],'wdf_basic'); ?> value="wdf_basic"><?php _e('Basic','wdf'); ?></option>
													<option <?php selected($settings['default_style'],'wdf_minimal'); ?> value="wdf_minimal"><?php _e('Minimal','wdf'); ?></option>
													<option <?php selected($settings['default_style'],'wdf_dark'); ?> value="wdf_dark"><?php _e('Dark','wdf'); ?></option>
													<option <?php selected($settings['default_style'],'wdf_note'); ?> value="wdf_note"><?php _e('Note','wdf'); ?></option>
													<option <?php selected($settings['default_style'],'wdf_custom'); ?> value="custom"><?php _e('None (Custom CSS)','wdf'); ?></option>
												</select>
											</td>
										</tr>
							...
							case 'payments' : ?>
								...
								<?php 
									// Temp fix for default settings error
									if(!is_array($settings['payment_types']))
										$settings['payment_types'] = array(); 
								?>
								...
								<table class="form-table">
									<tbody>
										...
										<tr valign="top">
											...
											<td>
												<input class="wdf_auto_submit" type="checkbox" name="wdf_settings[payment_types][]" value="advanced" <?php checked( in_array( 'advanced', $settings['payment_types'] ), true); ?> />
												<?php echo $tips->add_tip(__('Crowdfunding allows you to start your own crowdfunding resource similar to popular websites.','wdf')); ?>
											</td>
										</tr>
									</tbody>
								</table>
								<?php if(isset($settings['payment_types'])) : ?>
									
									<?php global $wdf_gateway_plugins, $wdf_gateway_active_plugins; ?>
									<?php //if($wdf_gateway_active_plugins) : ?>
